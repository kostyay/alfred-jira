...
const luminateAuth = require("../luminate-auth");
...
let Auth = {
  ...
  'login': function() {
    ...
    // get luminate token
    luminateAuth.getToken().then((x) => {
      config.req.headers['Lum-API-Token'] = x;
    })
  ...
  'setConfig': function () {
    ...
    return new Promise(function (resolve, reject) {
      if (self.checkConfig()) {
        ...
        // get luminate token to the request headers
        luminateAuth.getToken().then((x) => {
          if (x != null) {
            config.req.headers['Lum-API-Token'] = x;  
          }
